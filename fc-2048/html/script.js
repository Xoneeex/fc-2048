...
function drawBackground() {
    ...
    for (let i = 0; i < game.length; i++) {
        ...
        tileDiv.style.top = `${y * 144}px`;
        tileDiv.style.left = `${x * 144}px`;
...
function positionTile(tile, elm) {
    ...
    elm.style.top = `${y * 144}px`;
    elm.style.left = `${x * 144}px`;
}
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        if (tile) {
            if (isNew) {
                ...
                if(tile.value == 256) {
                    $.post(`https://${GetParentResourceName()}/gameWon`, JSON.stringify({}))
                    setTimeout(() => {
                        endDiv.innerText = "Hack Udany!"
                        endDiv.classList.add('active');
                        setTimeout(() => { udane(); }, 700);
                    }, 300);
                }
...
var seconds = 1000 * 59; //1000 = 1 second in JS
var timer;
...
function newGameStart() {
    ...
    endDiv.innerText = "Przygotuj się!";
    endDiv.classList.add('active');
    setTimeout(() => {
        ...
        seconds = 1000 * 59; //1000 = 1 second in JS
    }, 3000)
}
